...
import SeleccionarClientePorSede from "../clientes/SeleccionarClientePorSede";
...
const Ventas = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [categoriasData, sedesData] = await Promise.all([
          obtenerCategorias(),
          obtenerSedes(),
        ]);
  ...
  useEffect(() => {
    const fetchProductos = async () => {
      ...
      try {
        ...
        const productosData = await obtenerProductosPorSede(sede.id_sede);
        const transformados = productosData.map((p) => ({
          id: p.id_producto,
  ...
  const validarYProcesarVenta = async () => {
    ...
    try {
      setLoading(true);
      const inventario = await obtenerProductosPorSede(sede.id_sede);
  ...
  const buscarPorCodigo = async () => {
    ...
    try {
      ...
      // Llamada al service en lugar de usar axios directamente
      const producto = await obtenerProductoPorCodigo(codigoInput);
