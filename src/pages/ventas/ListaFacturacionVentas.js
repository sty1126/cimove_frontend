...
import {
  ...
  obtenerFacturasServicioTecnico,
...
const ListaFacturacionVentas = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const dataFacturas = await obtenerFacturasVenta();
      const facturasFormateadas = Array.isArray(dataFacturas)
        ? dataFacturas
        : [dataFacturas];
      setFacturas(facturasFormateadas);
      setFilteredFacturas(facturasFormateadas);

      // Cargar servicios técnicos facturados
      const dataServicios = await obtenerFacturasServicioTecnico();
      const serviciosFormateados = Array.isArray(dataServicios)
        ? dataServicios
        : [];
      setServiciosTecnicos(serviciosFormateados);
      setFilteredServicios(serviciosFormateados);
    } catch (error) {
