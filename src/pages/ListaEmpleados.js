...
const ListaEmpleados = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const activos = res.data.filter((emp) => emp.estado_empleado === "A");
      const inactivos = res.data.filter((emp) => emp.estado_empleado === "I");
  ...
  const handleReactivate = async (id) => {
    try {
      setLoading(true);
      const response = await fetch(
        `https://cimove-backend.onrender.com/api/empleados/restaurar/${id}`,
        {
          method: "PUT",
        }
      );
