...
const Catalogo = () => {
  ...
  const fetchSedes = async () => {
    try {
      ...
      if (sedesData.length > 0) {
        setSelectedSede(sedesData[0].nombre_sede);
      }
    } catch (err) {
  ...
  const fetchData = async () => {
    try {
      const general = await obtenerProductosGenerales();
      setGeneralProducts(formatData(general));

      const sedeObj = sedes.find((s) => s.nombre_sede === selectedSede);
      if (!sedeObj) return;

      const sedeProducts = await obtenerProductosPorSede(sedeObj.id_sede);
      setSedeProducts(formatData(sedeProducts));
    } catch (error) {
