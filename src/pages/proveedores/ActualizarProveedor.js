...
const ActualizarProveedor = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [tipos, ciudades, proveedor] = await Promise.all([
          obtenerTiposProveedor(),
          obtenerCiudades(),
        ...
        if (proveedor.fecharegistro_proveedor) {
          const fechaStr = proveedor.fecharegistro_proveedor.split("T")[0];
          fechaFormateada = dayjs(fechaStr);
        }

        setDigitoVerificacion(proveedor.digitoverificacion_proveedor || "");
  ...
  const handleSubmit = async (values) => {
    ...
    if (!validations.direccion.test(values.direccion_proveedor)) {
      message.error("La dirección contiene caracteres no permitidos");
      return;
    }

    if (!validations.telefono.test(values.telefono_proveedor)) {
      message.error("El teléfono debe contener entre 7 y 10 dígitos numéricos");
      return;
    }
    ...
    try {
      ...
      message.error({ content: msg, style: { marginTop: "20px" } });
    } finally {
      setSubmitting(false);
    }
  };
