...
import {
  obtenerFacturaConAbonos,
  registrarAbonoFactura,
...
const FormularioAbono = () => {
  ...
  const fetchFactura = async () => {
    setLoading(true);
    try {
      const data = await obtenerFacturaConAbonos(idFactura);
  ...
  const registrarAbono = async (values) => {
    setSubmitting(true);
    try {
      await registrarAbonoFactura(idFactura, values);
      message.success({
      ...
      message.error({
        content: error.message || "Error al registrar abono",
                      ...
                      <Col span={12}>
                        ...
                        <Text strong>
                          {factura.fecha_facturaproveedor
                            ? dayjs(factura.fecha_facturaproveedor).format(
                                "DD/MM/YYYY"
                              )
                            : "No disponible"}
                        </Text>
              ...
              <Col xs={24} sm={8}>
                <Card
                  style={{
                    ...
                    borderRadius: "8px",
                    borderTop: `3px solid ${
                      isPagado ? colors.success : colors.danger
                    }`,
                    boxShadow: "0 2px 8px rgba(0,0,0,0.05)",
                  }}
                  bodyStyle={{ padding: "16px" }}
                    ...
                    >
                      ...
                      >
                        <Space>
                          ...
                          <div>
                            <Text strong>
                              {formatCOP(abono.monto_abonofactura)}
                            </Text>
                            <br />
                            <Text type="secondary" style={{ fontSize: "12px" }}>
                              {dayjs(abono.fecha_abonofactura).format(
                                "DD/MM/YYYY"
                              )}
                            </Text>
                          </div>
              ...
              >
                  ...
                  <Col xs={24} md={12}>
                    <Form.Item
                      ...
                      rules={[
                        {
                          required: true,
                          message: "Ingrese el valor del abono",
                        },
                  ...
                  <Col xs={24} md={12}>
                    <Form.Item
                      ...
                      rules={[
                        {
                          required: true,
                          message: "Seleccione la fecha del abono",
                        },
                      ]}
                    >
                      <DatePicker
                ...
                <Form.Item style={{ marginTop: "24px", marginBottom: 0 }}>
                  <Button
                    ...
                    icon={
                      isPagado ? <CheckCircleOutlined /> : <DollarOutlined />
                    }
                  >
                    {isPagado
