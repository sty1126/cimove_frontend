...
// Servicios externos
import { obtenerSedes } from "../../services/clienteService";
import {
...
const CrearServicioTecnico = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setDataLoading(true);
        const { sedes, proveedores, tiposMetodoPago } =
          await obtenerDatosParaCrearServicio();
        setSedes(sedes);
        setProveedores(proveedores);
        setMetodosPago(tiposMetodoPago);
      } catch (error) {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      await registrarServicioTecnico(payload);
