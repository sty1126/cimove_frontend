...
import {
  obtenerServicioTecnicoConInfo,
  actualizarServicioTecnico,
} from "../../services/servicioTecnicoService";
...
const ActualizarServicioTecnico = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setDataLoading(true);
        const { sedes, proveedores, servicio, tiposMetodoPago } =
          await obtenerServicioTecnicoConInfo(id);

        setSedes(sedes);
        setProveedores(proveedores);
        setTiposMetodoPago(tiposMetodoPago);
        setServicioData(servicio);

        const totalAbono =
          servicio.metodos_pago?.reduce((sum, m) => sum + (m.monto || 0), 0) ||
          servicio.abono_serviciotecnico ||
          0;
        setMetodosPago(servicio.metodos_pago || []);
        setSaldoPendiente(servicio.costo_serviciotecnico - totalAbono);

        const sede = sedes.find(
          (s) => s.id_sede === Number(servicio.id_sede_serviciotecnico)
        );
        setSedeInfo(sede);

        setClienteInfo({ id_cliente: servicio.id_cliente_serviciotecnico });
        setAplicaProveedor(
    ...
    try {
      ...
      const payload = {
        ...
        fecha_entrega_serviciotecnico:
          values.fechaEntrega?.format("YYYY-MM-DD") || null,
      ...
      await actualizarServicioTecnico(id, payload);
