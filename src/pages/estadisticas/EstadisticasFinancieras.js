...
import { getAbonos } from "../../services/proveedoresService";
import {
  getIngresosTotales,
  getIngresosPorDia,
  getIngresosPorMes,
    ...
    const fetchData = async () => {
      ...
      try {
        const [
          ...
          getPagosProveedoresPorMes(),
          getNominaPorSedeYRol(),
          getAbonos(), // Este puede ir en abonosService.js
        ]);

        setIngresosTotales(ingresosTotalesRes.total_facturado);
        setIngresosPorDia(ingresosPorDiaRes);
        setIngresosPorMes(ingresosPorMesRes);
        setIngresosPorMetodoPago(ingresosPorMetodoPagoRes);
        setVentasPorSede(ventasPorSedeRes);
        setVentasPorSedePorMes(ventasPorSedePorMesRes);
        setVentasPorSedePorDia(ventasPorSedePorDiaRes);
        setClientesConPagosPendientes(clientesConPagosPendientesRes);
        setTopClientesPorMonto(topClientesPorMontoRes);
        setPagosProveedoresTotales(
          pagosProveedoresTotalesRes.total_pagado_proveedores
        );
        setPagosProveedoresPorMes(pagosProveedoresPorMesRes);
        setAbonos(abonosRes);

        const nominaData = nominaPorSedeYRolRes.map((item) => ({
        ...
        const totalProveedores = Number.parseFloat(
          pagosProveedoresTotalesRes.total_pagado_proveedores || 0
        );
