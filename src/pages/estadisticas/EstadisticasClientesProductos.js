  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Productos
        setProductosPorCantidad(productosPorCantidad || []);
        setProductosPorValor(productosPorValor || []);
        setProductosFrecuentes(productosFrecuentes || []);
        setStockVsVentas(stockVsVentas || []);
        setProductosObsoletos(productosObsoletos || []);

        // Clientes
        setClientesPorMonto(clientesPorMonto || []);
        setClientesPorCantidad(clientesPorCantidad || []);
        setClientesPorFrecuencia(clientesPorFrecuencia || []);
        setClientesFrecuentesVsEsporadicos(
          clientesFrecuentesVsEsporadicos || []
        );
        setClientesConPagosPendientes(clientesConPagosPendientes || []);
      } catch (error) {
