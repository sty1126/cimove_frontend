...
import { obtenerProductosPorSede } from "../../services/inventarioService";
import { obtenerNombreSede } from "../../services/productosService";
...
const SeleccionarProducto = ({ show, handleClose, setProducto, idSede }) => {
  ...
  useEffect(() => {
    const fetchProductos = async () => {
      if (!show || !idSede) return;

      setLoading(true);
      try {
        // Obtener productos de la sede
        const productos = await obtenerProductosPorSede(idSede);
        setProductos(productos);

        // Obtener nombre de la sede
        try {
          const sede = await obtenerNombreSede(idSede);
          setNombreSede(sede.nombre_sede || `Sede ${idSede}`);
        } catch (error) {
          console.error("Error al obtener nombre de sede", error);
          setNombreSede(`Sede ${idSede}`);
        }
      } catch (error) {
        console.error("Error al obtener productos", error);
        message.error("No se pudieron cargar los productos de la sede");
      } finally {
        setLoading(false);
      }
    };
  ...
  const productosFiltrados = productos.filter((p) => {
    const nombre = p.nombre_producto ? p.nombre_producto.toLowerCase() : "";
  ...
  return (
    ...
    >
      <Space align="center" style={{ marginBottom: "16px" }}>
        ...
        <Text strong>Sede Seleccionada:</Text>
        <Tag color={colors.primary} style={{ borderRadius: "4px" }}>
          {nombreSede || idSede || "Ninguna"}
        </Tag>
      </Space>
